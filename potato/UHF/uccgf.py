...
def greens_e_singles_ea_alpha(t1, t2, l1, l2, p):
    ...
    if p < nocca:
        ...
        result = np.zeros((nvira,), dtype=ds_type)
        result[p] = -1.0
        result += np.einsum('ia,i->a', l1a, t1a[:,p])
        result += 0.5*np.einsum('klca,klc->a', l2aa, t2aa[:,:,:,p])
        result += np.einsum('lkac,lkc->a', l2ab, t2ab[:,:,p,:])
        return result
...
def greens_e_singles_ea_beta(t1, t2, l1, l2, p):
    ...
    if p < noccb:
        ...
        result = np.zeros((nvirb,), dtype=ds_type)
        result[p] = -1.0
        result += np.einsum('ia,i->a', l1b, t1b[:,p])
        result += 0.5*np.einsum('klca,klc->a', l2bb, t2bb[:,:,:,p])
        result += np.einsum('klca,klc->a', l2ab, t2ab[:,:,:,p])
        return result
...
def greens_e_doubles_ea_alpha(t1, l1, l2, p):
    ...
    if p < nocca:
        ...
        evec_aaa[:,p,:] += -0.5*l1a
        evec_aaa[:,:,p] += 0.5*l1a
        evec_aaa += 0.5*np.einsum('k,jkba->jab',t1a[:,p], l2aa)
        evec_bab = np.zeros((noccb,nvira,nvirb), dtype=ds_type)
        evec_bab[:,p,:] += -l1b
        evec_bab += np.einsum('k,kjab->jab',t1a[:,p], l2ab)
...
def greens_e_doubles_ea_beta(t1, l1, l2, p):
    ...
    if p < noccb:
        ...
        evec_bbb[:,p,:] += -0.5*l1b
        evec_bbb[:,:,p] += 0.5*l1b
        evec_bbb += 0.5*np.einsum('k,jkba->jab',t1b[:,p], l2bb)
        evec_aba = np.zeros((nocca,nvirb,nvira), dtype=ds_type)
        evec_aba[:,p,:] += -l1a
        evec_aba += np.einsum('k,jkba->jab',t1b[:,p], l2ab)
...
def greens_e_singles_ip_alpha(t1, t2, l1, l2, p):
    ...
    if p < nocca:
        result = np.zeros((nocca,), dtype=ds_type)
        result[p] = -1.0
        result += np.einsum('ic,c->i',l1a,t1a[p,:])
        result += 0.5*np.einsum('ilcd,lcd->i',l2aa,t2aa[p,:,:,:])
        result += np.einsum('ilcd,lcd->i',l2ab,t2ab[p,:,:,:])
        return result
    else:
        p = p-nocca
        return -l1a[:,p]
...
def greens_e_singles_ip_beta(t1, t2, l1, l2, p):
    ...
    if p < noccb:
        result = np.zeros((noccb,), dtype=ds_type)
        result[p] = -1.0
        result += np.einsum('ic,c->i',l1b,t1b[p,:])
        result += 0.5*np.einsum('ilcd,lcd->i',l2bb,t2bb[p,:,:,:])
        result += np.einsum('lidc,ldc->i',l2ab,t2ab[:,p,:,:])
        return result
    else:
        p = p-noccb
        return -l1b[:,p]
...
def greens_e_doubles_ip_alpha(t1, l1, l2, p):
    ...
    if p < nocca:
        ...
        evec_aaa[p, :, :] += -0.5*l1a
        evec_aaa[:, p, :] += 0.5*l1a
        evec_aaa += 0.5*np.einsum('c,ijcb->ijb',t1a[p,:],l2aa)
        evec_abb = np.zeros((nocca, noccb, nvirb), dtype=ds_type)
        evec_abb[p, :, :] += -l1b
        evec_abb += np.einsum('c,ijcb->ijb',t1a[p,:],l2ab)
...
def greens_e_doubles_ip_beta(t1, l1, l2, p):
    ...
    if p < noccb:
        ...
        evec_bbb[p, :, :] += -0.5*l1b
        evec_bbb[:, p, :] += 0.5*l1b
        evec_bbb += 0.5*np.einsum('c,ijcb->ijb',t1b[p,:],l2bb)
        evec_baa = np.zeros((noccb, nocca, nvira), dtype=ds_type)
        evec_baa[p, :, :] += -l1a
        evec_baa += np.einsum('c,jibc->ijb',t1b[p,:],l2ab)
...
class UCCGF(object): 
    ...
    def ipccsd_ao(self, ps, omega_list, mo_coeff, broadening):
        ...
        e_vector_aoa = np.einsum("pi,ix->px", mo_coeff[0][ps,:], e_vector_moa)
        e_vector_aob = np.einsum("pi,ix->px", mo_coeff[1][ps,:], e_vector_mob)
        ...
        b_vector_aoa = np.einsum("xi,ip->xp", b_vector_moa, mo_coeff[0].T[:,ps])
        b_vector_aob = np.einsum("xi,ip->xp", b_vector_mob, mo_coeff[1].T[:,ps])

        gf_ao = np.zeros((2, len(ps), len(ps), len(omega_list)), dtype=np.complex128)
        for ip, p in enumerate(ps):
            b1a = b_vector_aoa[:,p][:nocca]
            b2aaa = b_vector_aoa[:,p][nocca:nocca+nocca*nocca*nvira].reshape(nocca,nocca,nvira)
            b2abb = b_vector_aoa[:,p][nocca+nocca*nocca*nvira:].reshape(nocca,noccb,nvirb)
            b1b = b_vector_aob[:,p][:noccb]
            b2baa = b_vector_aob[:,p][noccb:noccb+noccb*nocca*nvira].reshape(noccb,nocca,nvira)
            b2bbb = b_vector_aob[:,p][noccb+noccb*nocca*nvira:].reshape(noccb,noccb,nvirb)
            b1 = (b1a, b1b)
    ...
    def eaccsd_ao(self, ps, omega_list, mo_coeff, broadening):
        ...
        gf_ao = np.zeros((2, len(ps), len(ps), len(omega_list)), dtype=np.complex128)
        for ip, p in enumerate(ps):
            b1a = b_vector_aoa[:,p][:nvira]
            b2aaa = b_vector_aoa[:,p][nvira:nvira+nvira*nocca*nvira].reshape(nocca,nvira,nvira)
            b2bab = b_vector_aoa[:,p][nvira+nvira*nocca*nvira:].reshape(noccb,nvira,nvirb)
            b1b = b_vector_aob[:,p][:nvirb]
            b2aba = b_vector_aob[:,p][nvirb:nvirb+nocca*nvirb*nvira].reshape(nocca,nvirb,nvira)
            b2bbb = b_vector_aob[:,p][nvirb+nocca*nvirb*nvira:].reshape(noccb,nvirb,nvirb)
            b1 = (b1a, b1b)
