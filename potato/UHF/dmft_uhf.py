...
np.set_printoptions(7, linewidth=1000)
...
def kernel_mu(dmft, mu, delta=None, sigma0=None,
    ...
    # sigma spin-unrestricted
    if sigma0 is None:
        sigma = dmft.get_init_sigma()
    else:
        assert (sigma0.shape == (2,nao,nao,nw))
        sigma = sigma0.copy()

    # GF spin-unrestricted
    gf0_cell = get_gf([hcore_cell, hcore_cell], sigma, freqs, delta)
    gf_cell = np.zeros([2, nao, nao, nw], np.complex128)
    for k in range(nkpts):
        gf_cell += 1./nkpts * get_gf([hcore_k[k,:,:],hcore_k[k,:,:]], sigma, freqs, delta)
    ...
    while not dmft_conv and cycle < max(1, dmft.max_cycle):
        hyb_last = hyb.copy()
        bath_v_a, bath_e_a = get_bath(hyb[0], freqs, wts)
        himp_a, eri_imp_a = imp_ham(hcore_cell, eris, bath_v_a, bath_e_a)
        bath_v_b, bath_e_b = get_bath(hyb[1], freqs, wts)
        himp_b, eri_imp_b = imp_ham(hcore_cell, eris, bath_v_b, bath_e_b)

        dmft._scf = mf_kernel(himp_a, eri_imp_a, himp_b, eri_imp_b, mu, verbose=dmft.verbose)
        gf_imp = dmft.get_gf_imp(freqs, delta)
        gf_imp = gf_imp[:,:nao,:nao,:]
        # Symmetrize CCGF
        for i in range(2):
            gf_imp[i] = 0.5*(gf_imp[i]+gf_imp[i].transpose(1,0,2))

        nb = bath_e_a.shape[0]
        sgdum = np.zeros((2,nb+nao,nb+nao,nw))
        gf0_imp = get_gf([himp_a, himp_b], sgdum, freqs, delta)
...
def get_sigma(mf_gf, corr_gf):
    ...
    for i in range(2):
        for iw in range(nw):
            sigma[i,:,:,iw] = linalg.inv(mf_gf[i,:,:,iw]) - linalg.inv(corr_gf[i,:,:,iw])
    return sigma

def mf_kernel(himp_a, eri_imp_a, himp_b, eri_imp_b, mu, verbose=logger.NOTE):
    n = himp_a.shape[0]
    mol = gto.M()
    mol.verbose = verbose
    mol.spin = 0
    ...
    mf = scf.UHF(mol, mu, np.asarray([himp_a,himp_b]))
    mf.max_memory = 1000
    mf.mo_energy = [np.zeros(n),np.zeros(n)]
    mf.conv_tol = 1e-10

    mf.get_hcore = lambda *args: himp_a
    mf.get_ovlp = lambda *args: np.eye(n)
    mf._eri = ao2mo.restore(8, eri_imp_a, n)
    mf.init_guess = '1e'  # currently needed
    dm_alpha, dm_beta = mf.get_init_guess()
    for i in range(dm_alpha.shape[0]):
        dm_beta[i,i] = dm_alpha[i,i]
    # Initial guess for AFM 2D Hubbard
    dm_alpha[0,0] = 0.7; dm_alpha[1,1] = 0.3; dm_alpha[2,2] = 0.3; dm_alpha[3,3] = 0.7
    dm_beta[0,0] = 0.3; dm_beta[1,1] = 0.7; dm_beta[2,2] = 0.7; dm_beta[3,3] = 0.3
    dm = (dm_alpha,dm_beta)
...
def ucc_gf(mf, freqs, delta, ao_orbs=None, gmres_tol=1e-4):
    ''' Calculate the UCCSD GF matrix in the AO basis'''
    if ao_orbs is None:
    ...
    mycc.solve_lambda()
    dm = mycc.make_rdm1()
    rdm1 = numpy.dot(mf.mo_coeff[0], numpy.dot(dm[0], mf.mo_coeff[0].T))
    rdm2 = numpy.dot(mf.mo_coeff[1], numpy.dot(dm[1], mf.mo_coeff[1].T))
...
def get_gf(hcore, sigma, freqs, delta):
    ...
    Args:
         hcore : (2, nao, nao) ndarray
         sigma : (2, nao, nao) ndarray
         freqs : (nw) ndarray
         delta : float

    Returns:
         gf : (2, nao, nao) ndarray

    """
    nw  = len(freqs)
    nao = hcore[0].shape[0]
    gf = np.zeros([2, nao, nao, nw], np.complex128)
    for i in range(2):
        for iw, w in enumerate(freqs):
            gf[i,:,:,iw] = linalg.inv((w+1j*delta)*np.eye(nao)-hcore[i]-sigma[i,:,:,iw])
    return gf
...
def get_bath(hyb, freqs, wts):
    ...
    for iw in range(nw):
        ...
        # manully set negtive eigs to zero
...
class DMFT(lib.StreamObject):
    ...
    def get_init_sigma(self, freqs=None):
        ...
        sigma = np.empty([2,nao,nao,nbath], dtype=np.complex)
        U = self.eris[0,0,0,0]
        if freqs is None:
            freqs = self.freqs
        ...
        for iw,w in enumerate(freqs):
            sigma[0,:,:,iw] = U*occupancy/2.*np.eye(nao)
            sigma[1,:,:,iw] = U*occupancy/2.*np.eye(nao)
#        for iw,w in enumerate(freqs):
#            sigma[0,0,0,iw] = 0.4*U; sigma[0,3,3,iw] = 0.4*U
#            sigma[0,1,1,iw] = 0.6*U; sigma[0,2,2,iw] = 0.6*U
#            sigma[1,0,0,iw] = 0.6*U; sigma[1,3,3,iw] = 0.6*U
#            sigma[1,1,1,iw] = 0.4*U; sigma[1,2,2,iw] = 0.4*U
#            for i in range(nao):
#                if i%2 == 0:
#                    sigma[0,i,i,iw] = U*occupancy*0.4
#                    sigma[1,i,i,iw] = U*occupancy*0.6
#                else:
#                    sigma[0,i,i,iw] = U*occupancy*0.6
#                    sigma[1,i,i,iw] = U*occupancy*0.4
        return sigma
    ...
    def get_rdm_imp(self):
        ...
        if self.solver_type == 'scf':
            ...
            return ucc_rdm(self._scf, ao_orbs=range(self.nao))
    ...
    def get_gf_imp(self, freqs, delta):
        ...
        if self.solver_type == 'scf':
            ...
            return ucc_gf(self._scf, freqs, delta, ao_orbs=range(self.nao),
                         gmres_tol=self.gmres_tol)
        elif self.solver_type == 'fci':
    ...
    def get_gf0_imp(self, freqs, delta):
        ...
        himp_a, himp_b = self._scf.h1e_spin
        nb = himp_a.shape[0]
        nw = len(freqs)
        sig_dum = np.zeros((2,nb,nb,nw,))
        gf = get_gf([himp_a,himp_b], sig_dum, freqs, delta)
    ...
    def get_ldos_imp(self, freqs, delta):
        ...
        ldos = np.zeros([2,nao, nw])
        for p in range(nao):
           ldos[0,p,:] += -1./np.pi * np.imag((gf[0])[p,p,:])
           ldos[1,p,:] += -1./np.pi * np.imag((gf[1])[p,p,:])
        return ldos
    ...
    def _gf (self, freqs, delta):
        ...
        if self.solver_type == 'scf':
            ...
            gf = ucc_gf (freqs, delta, self.corr_, self.mf_.mo_coeff)
